    ...
    public void WebWxBatchGetContact() {
        try {
            String url = String.format(URLEnum.WEB_WX_BATCH_GET_CONTACT.getUrl(),
                    ...
                    core.getLoginInfo().get(StorageLoginInfoEnum.pass_ticket.getKey()));
            int batchSize = 30; // 每次请求30个
            int totalSize = core.getGroupIdList().size();
            int startIndex = 0;
            while (startIndex < totalSize) {
                // 计算当前请求的结束索引
                int endIndex = Math.min(startIndex + batchSize, totalSize);
                // 从groupIdList中提取当前批次的ID
                List<String> currentBatchGroupIds = core.getGroupIdList().subList(startIndex, endIndex);
                Map<String, Object> paramMap = core.getParamMap();
                paramMap.put("Count", currentBatchGroupIds.size());
                List<Map<String, String>> list = new ArrayList<Map<String, String>>();
                for (int i = 0; i < currentBatchGroupIds.size(); i++) {
                    HashMap<String, String> map = new HashMap<String, String>();
                    map.put("UserName", currentBatchGroupIds.get(i));
                ...
                for (int i = 0; i < contactList.size(); i++) { // 群好友
                    if (contactList.getJSONObject(i).getString("UserName").indexOf("@@") > -1) { // 群
                        ...
                        core.getGroupIdNameMap().put(contactList.getJSONObject(i).getString("NickName"), contactList.getJSONObject(i).getString("UserName"));
                    }
                }
                // 更新startIndex，处理下一批
                startIndex = endIndex;
        ...
        LOG.info("group size:" + core.getGroupList());
